    ...
    return (
        <div className="home">
            <Slider {...carouselSettings} className="carousel">
                {meals.slice(0, 12).map(meal => ( // Show first 12 meals in carousel
                    <div key={meal.idMeal} className="carousel-item">
                        <Link to={`/meal/${meal.idMeal}`}>
                            <img src={meal.strMealThumb} alt={meal.strMeal} className="carousel-image"/>
                        </Link>
            ...
            <div className="meal-container">
                {meals.slice(0, visibleCount).map(meal => ( // Show limited number of meals
                    <div className="meal-card" key={meal.idMeal}>
                        <Link to={`/meal/${meal.idMeal}`} className="meal-link">
                            <img src={meal.strMealThumb} alt={meal.strMeal} className="meal-image"/>
                        </Link>
                        <FavoriteButton /> {/* Moved right after image */}
            ...
            {visibleCount < meals.length && ( // Show "Show More" button only if there are more items to display
                <button onClick={showMoreItems} className="show-more-btn">Show More</button>
            )}

            {showScrollButtons && (
                <div className="scroll-buttons">
                    <button onClick={scrollToTop} className="scroll-button scroll-up">↑</button>
                    <button onClick={scrollToBottom} className="scroll-button scroll-down">↓</button>
                </div>
            )}
        </div>
    );
